...
if($id){
    $q = mysqli_fetch_assoc(mysqli_query($db, "SELECT * FROM quiz_questions WHERE id=$id"));
    $quiz_id = $q['quiz_id'];
    $question = $q['question'];
...
if($_SERVER['REQUEST_METHOD']=='POST'){
    ...
    if($id){
        mysqli_query($db, "UPDATE quiz_questions SET question='$question', question_type='$question_type', points=$points, order_index=$order_index WHERE id=$id");
    } else {
        mysqli_query($db, "INSERT INTO quiz_questions (quiz_id, question, question_type, points, order_index) VALUES ($quiz_id, '$question', '$question_type', $points, $order_index)");
    }
...
<div class="container mt-4">
    ...
    <form method="post">
        ...
        <div class="mb-3">
            <label>Points</label>
            <input type="number" name="points" class="form-control" value="<?= $points ?>">
        </div>
        <div class="mb-3">
            <label>Order Index</label>
            <input type="number" name="order_index" class="form-control" value="<?= $order_index ?>">
        </div>
        <button class="btn btn-success"><?= $id ? 'Update' : 'Save' ?></button>
        <a href="questions.php?quiz_id=<?= $quiz_id ?>" class="btn btn-secondary">Back</a>
    </form>
</div>
