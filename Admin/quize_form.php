...
$id = $_GET['id'] ?? '';
$title = $description = $passing_score = $time_limit = $is_active = '';

if($id){
    $quiz = mysqli_fetch_assoc(mysqli_query($db, "SELECT * FROM quizzes WHERE id=$id"));
    $title = $quiz['title'];
...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if($id){
        mysqli_query($db, "UPDATE quizzes SET title='$title', description='$description', passing_score='$passing_score', time_limit='$time_limit', is_active='$is_active' WHERE id=$id");
    } else {
        mysqli_query($db, "INSERT INTO quizzes(title, description, passing_score, time_limit, is_active) VALUES('$title','$description','$passing_score','$time_limit','$is_active')");
    }
    header("Location:manage_quize.php");
}
?>
<!DOCTYPE html>
<html lang="en"
...
<div class="container mt-4">
    <h2><?= $id ? 'Edit' : 'Add' ?> Quiz</h2>
    <form method="post">
        <div class="mb-3">
            <label>Title</label>
            <input type="text" name="title" class="form-control" required value="<?php $title ?>">
        </div>
        <div class="mb-3">
            <label>Description</label>
            <textarea name="description" class="form-control"><?php $description ?></textarea>
        </div>
        <div class="mb-3">
            <label>Passing Score</label>
            <input type="number" name="passing_score" class="form-control" value="<?php $passing_score ?>">
        </div>
        <div class="mb-3">
            <label>Time Limit (minutes, 0 = no limit)</label>
            <input type="number" name="time_limit" class="form-control" value="<?php $time_limit ?>">
        </div>
        <div class="mb-3">
            <label>Active</label>
        ...
        <a href="create_quize.php" class="btn btn-secondary">Back</a>
    </form>
</div>
