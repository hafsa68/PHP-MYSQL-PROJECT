...
if($id){
    $a = mysqli_fetch_assoc(mysqli_query($db, "SELECT * FROM quiz_answers WHERE id=$id"));
...
if($_SERVER['REQUEST_METHOD']=='POST'){
    ...
    if($id){
        mysqli_query($db, "UPDATE quiz_answers SET answer_text='$answer_text', is_correct=$is_correct WHERE id=$id");
    } else {
        mysqli_query($db, "INSERT INTO quiz_answers (question_id, answer_text, is_correct) VALUES ($question_id, '$answer_text', $is_correct)");
    }
...
<div class="container mt-4">
    <h2><?= $id ? 'Edit' : 'Add' ?> Answer</h2>
    <form method="post">
        <div class="mb-3">
            <label>Answer Text</label>
            <textarea name="answer_text" class="form-control" required><?= $answer_text ?></textarea>
        </div>
        <div class="form-check mb-3">
            <input type="checkbox" class="form-check-input" name="is_correct" id="is_correct" <?= $is_correct ? 'checked' : '' ?>>
            <label class="form-check-label" for="is_correct">Correct Answer</label>
        </div>
        <button class="btn btn-success"><?= $id ? 'Update' : 'Save' ?></button>
        <a href="answers.php?question_id=<?= $question_id ?>" class="btn btn-secondary">Back</a>
    </form>
</div>
